...
class BSUpdateShopperTests: XCTestCase {
    ...
    func testUpdateShopperApplePay() {
        ...
        guard let vaultedShopperId = BSIntegrationTestingAPIHelper.createVaultedShopper(creditCard: cc, set2: set2) else {
    ...
    func testUpdateShopperPayPal() {
        var set2: Bool = false
        let cc: [String: String] = BluesnapSDKIntegrationTestsHelper.getMasterCard()
        guard let vaultedShopperId = BSIntegrationTestingAPIHelper.createVaultedShopper(creditCard: cc, set2: set2) else {
            XCTFail("Failed to create vaulted shopper"); return
        }
