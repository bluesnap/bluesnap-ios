...
class BSIntegrationTestingAPIHelper {
    ...
    static func createToken(shopperId: Int?, completion: @escaping (BSToken?, BSErrors?) -> Void) {
        createSandboxBSToken(shopperId: shopperId, completion: { bsToken, bsError in
            ...
            do {
                ...
                XCTFail("createToken: unexpected error: \(error)")
                completion(nil, bsError)
            }
        })
    ...
    static func createVaultedShopper(creditCard: [String: String], set2: Bool = false, storeCard:Bool = true) -> Int? {
        ...
        var vaultedShopperId: Int? = nil

        let semaphore = DispatchSemaphore(value: 0)
        BSIntegrationTestingAPIHelper.createToken(completion: { token, error in
            guard let token = token, let tokenStr = token.getTokenStr() else {
                XCTFail("createVaultedShopper: createToken failed, error: \(String(describing: error))")
                semaphore.signal()
                return
            }
            bsToken = BluesnapSDKIntegrationTestsHelper.initBSToken(tokenStr: tokenStr)
        ...
        BSIntegrationTestingAPIHelper.createTokenizedTransaction(purchaseAmount: 10, purchaseCurrency: "USD", bsToken: bsToken, completion: { isSuccess, data, shopperId in
            XCTAssert(isSuccess, "createTokenizedTransaction failed")
            if let shopperId = shopperId, let id = Int(shopperId) {
                vaultedShopperId = id
            } else {
                XCTFail("createVaultedShopper: no shopperId returned from transaction")
            }
            semaphore2.signal()
        })
        semaphore2.wait()
    ...
    static func submitCCDetails(ccDetails: [String: String], billingDetails: BSBillingAddressDetails?, shippingDetails: BSShippingAddressDetails?, storeCard: Bool, completion: @escaping (BSErrors?) -> Void) {
        BSApiManager.submitPurchaseDetails(ccNumber: ccDetails["ccn"], expDate: ccDetails["exp"], cvv: ccDetails["cvv"],
            ...
            if let error = error {
                XCTFail("submitCCDetails failed with error: \(error)")
                completion(error)
                return
            }
            guard let ccType = result.ccType, let last4 = result.last4Digits, let country = result.ccIssuingCountry else {
                XCTFail("submitCCDetails returned nil result fields")
                completion(error)
                return
            }
            NSLog("Result: ccType=\(ccType), last4Digits=\(last4), ccIssuingCountry=\(country)")
            XCTAssertEqual(last4, ccDetails["last4Digits"], "last4 should be \(ccDetails["last4Digits"] ?? "nil")")
            XCTAssertEqual(ccType, ccDetails["ccType"], "CC Type should be \(ccDetails["ccType"] ?? "nil")")
            XCTAssertEqual(country, ccDetails["issuingCountry"], "country should be \(ccDetails["issuingCountry"] ?? "nil")")
            completion(error)
        })
